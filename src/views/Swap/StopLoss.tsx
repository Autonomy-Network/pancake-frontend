...
import { ArrowWrapper, ErrorText, SwapCallbackError, Wrapper } from './components/styleds'
...
export default function StopLoss({ history }: RouteComponentProps) {
  ...
  }, [independentField, parsedAmount, trade, showWrap])
  ...
  const onStopLossValuesChange = (source: string, value: string) => {
    ...
    if (source === 'price') {
      ...
      setStopLossPrice(price === Infinity || Number.isNaN(price) ? '' : price.toFixed(2))
    }
  }
  ...
  const realPriceValue = useMemo(() => {
    if (inputFocused) {
      ...
      return price === Infinity || Number.isNaN(price) ? '' : price.toFixed(2)
    }
  ...
  return (
    <Page>
      <AppBody>
        <AppHeader title={t('Stop Loss')} subtitle={t('Powered by Autonomy Network')} />
        <Wrapper id="swap-page">
          <AutoColumn gap="md">
            ...
            <CurrencyInputPanel
              ...
              label={independentField === Field.INPUT && !showWrap && trade ? t('Sell To (estimated)') : t('Sell To')}
              showMaxButton={false}
